...
class MyOroshiuriLib():
    browser = None#ブラウザーの表示・操作用
    wait = None#メルカリにアクセスする前に前回のアクセスからのインターバルを設定する
    DEBUGMODE = False

    # initialize-
    def __init__(self, browser, EBUGMODE = False,**kwargs):
        self.browser = browser
        self.DEBUGMODE = EBUGMODE
    ...
    # destructor
    def __del__(self):
        if self.DEBUGMODE: print("MyOroshiuriLib __del__:")
        pass

    #ログイン中かチェック。ログイン出来たらTrueを返す。出来なかったらFalseを返す.waitに指定がある場合はwait秒ログインチェックを繰り返す
    def loginCheck(self, wait = 60):
        self.browser.set_URL("https://oroshi-uri.com/")
        sleep(1)#リダイレクトを待つ
        #もし全画面広告が出たらスキップ
        elements = self.browser.find_elementsByXpath("//button[@aria-label='Close (Press escape to close)']")
        if len(elements) > 0:
            elements[0].click()
            sleep(1)  # 処理待ち
        ...
        for count in range(wait):
            #マイページが無ければログイン出来ていない
            elements = self.browser.find_elementsByXpath("//li[@class='__logout']")
            if len(elements) < 1:
                if print_flug:
                    print("\033[34m■■■■■■卸売りネットへ未ログインです。ログインをしてください■■■■■■\033[0m")
                    print_flug = False
                    self.browser.set_URL("https://oroshi-uri.com/login.php")
                else:
                    print(".",end="")

            else:
                print("\033[32m卸売りネットへログイン済みです\033[0m")
                break
    ...
    def getPriceByJancode(self, jancode):
        #print(f"{jancode}で卸売りドットコムを検索します。")

        # ①jancodeから価格(最安値,リンク)を取得※期間限定2重価格の場合は元の価格
        price = [999999999, ""]

        # JANコードで検索
        self.browser.set_URL("https://oroshi-uri.com/list.php?keyword=" + jancode)
