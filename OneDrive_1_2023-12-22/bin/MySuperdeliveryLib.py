...
class MySuperdeliveryLib():
    ...
    # destructor
    def __del__(self):
        if self.DEBUGMODE: print("MySuperdeliveryLib __del__:")
        pass

    #ログイン中かチェック。ログイン出来たらTrueを返す。出来なかったらFalseを返す.waitに指定がある場合はwait秒ログインチェックを繰り返す
    def loginCheck(self, wait = 60):
        self.browser.set_URL("https://www.superdelivery.com/")
        sleep(1)#リダイレクトを待つ

        print_flug = True

        for count in range(wait):
            #利用可能ポイントが無ければログイン出来ていない
            elements = self.browser.find_elementsByXpath("//div[@id='point-num1']")
            if len(elements) < 1:
                if print_flug:
                    print("\033[34m■■■■■■スーパーデリバリーへ未ログインです。ログインをしてください■■■■■■\033[0m")
                    print_flug = False
                    self.browser.set_URL("https://www.superdelivery.com/p/do/clickMemberLogin")
                else:
                    print(".",end="")

            else:
                print("\033[32mスーパーデリバリーへログイン済みです\033[0m")
                break
    ...
    def getPriceByJancode(self, jancode):
        #print(f"{jancode}でスーパーデリバリーを検索します。")

        # ①jancodeから価格(最安値,リンク)を取得※期間限定2重価格の場合は元の価格
        price = [999999999, ""]

        # JANコードで検索
        self.browser.set_URL("https://www.superdelivery.com/p/do/psl/?word=" + jancode)
        # 入れ物を取得
        elements = self.browser.find_elementsByXpath("//div[@class= 'dealer-itembox-inner item-scroll-box']")
        ...
        for element in elements:
            # 価格の取得
            price_elements = element.find_elements(By.CLASS_NAME, "item-price")
